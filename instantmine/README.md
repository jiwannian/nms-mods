...
MBIN 不能热加载，开关靠改正在运行的 `NMS.exe` 内存。

1. 先装好本 mod 并进存档。
2. 运行 `Start-InstantMineToggle.cmd`（或 `python instantmine_toggle.py`），窗口保持开着。
3. NMS 窗口在前台时按 **F8** 切换开/关。关的时候采矿恢复原版手感，载具激光伤害一并还原。
4. 热键在 `CONFIG.ini` 的 `[Toggle] Hotkey=`，只支持 F1–F12。

第一次定位会扫模块和映射文件（通常几秒），成功后写入 `instantmine_toggle.cache.json`。同一局游戏里重开开关、或重启游戏但版本没变时，会先校验缓存，对得上就跳过全扫。游戏更新或字段对不上才会重新扫。只改对得上整段特征的字段，避免把无关的 `0.01` 写成激光速度。若关开关后光束消失、矿物不掉血：完全退出游戏再进档（被写坏的是进程内存，重载 MBIN 会恢复）。

托盘/控制台会提示当前是开还是关。完全退出游戏后脚本会自动重新附加。Ctrl+C 结束脚本。
